    ...
    <head>
		...
		<style>
			...
			@media only screen and (max-width: 767px){
				...
				.main-work-section{margin:0;}
				.table input[type="text"], .table input[type="password"], .select {
 
				  width: 51%;
				}
				.work-here > form > input[type="submit"] {
					margin-right:168px;
				}
	...
	<div class="main-work-section clear">
		...
		<?php
			if($_SERVER['REQUEST_METHOD']=='POST'){
				...
				if(empty($id) or empty($prog) or empty($pass)){
					echo "Field must not blank.";
				}
				else{
					
					$login = $user->loginuser($id,$prog,$pass);
					if($login){
						header('Location: profile.php');
						ob_end_flush();
					}
			...
			<form action="#" method="post">
				<table class="table clear">
					...
					<tr>
						<td>Program</td>
						<td>
							<select name="prog" class="select clear">
								<option> BCSE</option> 
								<option> BSEEE</option> 
								<option >BSME</option> 
								<option >BSAg</option> 
